...
export const SkillCard: React.FC<SkillCardProps> = ({ skill, index, cardVariants }) => (
  ...
  >
    <div
      className="bg-white dark:bg-[#23272f] border border-gray-200 dark:border-[#FD9800] rounded-xl p-6 transition-all duration-300 shadow-sm flex flex-col h-full"
    >
      <div className="flex items-start space-x-4 mb-4">
        <div className="flex-shrink-0 text-3xl">{skill.logo}</div>
        <div className="flex-1">
          <h4 className="text-lg font-bold text-gray-900 dark:text-[#FD9800] mb-1">{skill.name}</h4>
